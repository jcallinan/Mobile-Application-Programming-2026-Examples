...
import { deleteIdea, initializeIdeaDatabase, insertIdea, listIdeas, replaceIdeas } from '../services/ideaDatabase';
...
export function IdeaVaultScreen() {
  ...
  const averageRating = useMemo(() => {
    ...
    return (ideas.reduce((sum, idea) => sum + idea.rating, 0) / ideas.length).toFixed(1);
  }, [ideas]);

  const saveDraft = () => {
    const nextDraft = sanitizeIdeaDraft(draft);
    if (!nextDraft.title || !nextDraft.notes) {
      Alert.alert('Missing info', 'Add at least a title and notes before saving.');
      return;
    }
  ...
  const handleImport = async () => {
    try {
      ...
      refreshIdeas();
      Alert.alert('Import complete', `Loaded ${importedIdeas.length} ideas from backup.`);
    } catch (error) {
  ...
  const handleExport = async () => {
    try {
      const fileUri = await exportIdeas(ideas);
      Alert.alert('Export ready', `Backup saved to ${fileUri}`);
    } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container}>
        <ScreenHeader count={ideas.length} averageRating={averageRating} />
